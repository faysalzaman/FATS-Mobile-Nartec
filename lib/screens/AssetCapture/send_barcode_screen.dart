...
import 'package:fats_mobile_nartec/screens/home_screen.dart';
...
class _SendBarCodeScreenState extends State<SendBarCodeScreen> {
  ...
  // Add these as class fields to store the original unfiltered lists
  List<String> _originalCategoryList = [];
  List<String> _originalSCode = [];
  List<String> _originalMCode = [];
  List<String> _originalMainDescription = [];
  List<String> _originalSubDescription = [];
    ...
    Future.delayed(Duration.zero, () {
      ...
      GetAllCategoriesServices.getAllCategories().then((value) async {
        setState(() {
          ...
          // Store original lists after initial load
          _originalCategoryList = List.from(categoryList);
          _originalSCode = List.from(sCode);
          _originalMCode = List.from(mCode);
          _originalMainDescription = List.from(majorDescription);
          _originalSubDescription = List.from(minorDescription);
        ...
        GetBrandServices.getBrandMethod(_sCode, _mCode).then((value) {
          setState(() {
            ...
            selectBrand = brandList.isNotEmpty ? brandList[0] : "Select Brand";
          });
  ...
  void filterCategoryList() {
    if (filterByController.text.isEmpty) {
      // Restore original lists instead of making API call
      setState(() {
        categoryList = List.from(_originalCategoryList);
        sCode = List.from(_originalSCode);
        mCode = List.from(_originalMCode);
        majorDescription = List.from(_originalMainDescription);
        minorDescription = List.from(_originalSubDescription);
    ...
    // Find matching indices from original list
    for (int i = 0; i < _originalCategoryList.length; i++) {
      if (_originalCategoryList[i]
          .toLowerCase()
    ...
    // Update all lists using matched indices
    setState(() {
      categoryList =
          matchedIndices.map((i) => _originalCategoryList[i]).toList();
      sCode = matchedIndices.map((i) => _originalSCode[i]).toList();
      mCode = matchedIndices.map((i) => _originalMCode[i]).toList();
      majorDescription =
          matchedIndices.map((i) => _originalMainDescription[i]).toList();
      minorDescription =
          matchedIndices.map((i) => _originalSubDescription[i]).toList();
                              ...
                              onChanged: (value) async {
                                ...
                                GetBrandServices.getBrandMethod(_sCode, _mCode)
                                  ...
                                  setState(() {
                                    ...
                                    brandList = brandList.toSet().toList();
                                    selectBrand = brandList.isNotEmpty
                                        ? brandList[0]
                                        : "Select Brand";
                                  });
                                  Navigator.pop(context);
